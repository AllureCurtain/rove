...
fn plan_decision_label(kind: rove_runtime::execution::PlanDecisionKind) -> &'static str {
    match kind {
        rove_runtime::execution::PlanDecisionKind::Continue => "continue",
        rove_runtime::execution::PlanDecisionKind::ReplaceRemaining => "replace remaining",
        rove_runtime::execution::PlanDecisionKind::Finish => "finish",
    }
}
    ...
    for tool in &run.tool_calls {
        ...
        if let Some(output) = &tool.output {
            push_labeled_text(
                lines,
                "  Output",
                &sanitize_tool_text(output, crate::tui::state::MAX_TOOL_DETAIL_TEXT_BYTES),
                Style::default().fg(Color::DarkGray),
            );
        }
        if let Some(error) = &tool.error {
            push_labeled_text(
                ...
                &sanitize_tool_text(
                    &error.to_string(),
                    crate::tui::state::MAX_TOOL_DETAIL_TEXT_BYTES,
                ),
...
fn sanitize_user_text(value: &str) -> String {
    truncate_display_text(
        &sanitize_display_text(value, crate::tui::state::MAX_TOOL_DETAIL_TEXT_BYTES),
        crate::tui::state::MAX_TOOL_DETAIL_TEXT_BYTES,
    )
}
