...
`main` is already beyond the original M0-M2 kernel plan and includes partial M3 plus substantial M4, M5, and M6 surfaces. The largest difference is shape: the original docs describe an eventual `EngineDeps` dependency graph and richer provider routing, while the current implementation remains a simpler single-crate engine. `RunStream`, cooperative cancellation, CLI signal cancellation, ToolContext cancellation propagation, ToolContext input providers, engine-level input-provider wiring, CLI stdin `request_input`, API pending-input `request_input` transport, web input-response UI, post-run hook cancellation, prompt-time durable memory, prompt-time session memory file loading, default post-run session summary writes, native Anthropic/Ollama adapters, stdio/SSE MCP proxy transport, and a basic routing model client with fallback model-id config, first-chunk probing, in-memory circuit state, configurable health policy, OpenAI-compatible fallback provider configs, basic `ModelError` classification, and OpenAI-compatible HTTP error mapping now exist. The documented M6 Web UI deliverables are implemented; the largest remaining gaps are post-M6/runtime-deepening work such as the full app-level token tree, layered memory store, durable-memory promotion hooks, user-configured post-run hooks, and structured provider-native tool-use normalization.
...
| Web workbench | Next.js workbench consumes SSE and shows plan/tools/trace | `web-ui/app/page.tsx`, `web-ui/components/rove-workbench.tsx`, `web-ui/lib/rove-state.ts`, `web-ui/lib/rove-state.test.ts` | Current UI includes chat, plan, tools, trace, cancel, approval buttons, pending-input submission, API job-state sync, and reducer tests. |
...
| M4 MCP transport | JSON-RPC over stdio/SSE | Stdio JSON-RPC plus SSE endpoint discovery/request transport | Mock-server tests cover stdio; the SSE proxy path is implemented in `src/tools/mcp_proxy.rs`, with broader real-server validation still open. |
| Model providers | OpenAI, Anthropic, Ollama, DeepSeek, routing/fallback | OpenAI-compatible, native Anthropic, native Ollama, and fake model clients | DeepSeek and other compatible providers can still be configured through OpenAI-compatible bases; native Anthropic/Ollama adapters now exist. |
| Routing model client | Provider fallback with TTFB probe and circuit breaker | `src/models/routing.rs` adds a reusable fallback client with configurable first-chunk probe timeout and in-memory CLOSED/OPEN/HALF_OPEN circuit state; `ROVE_FALLBACK_MODELS`, `ROVE_FALLBACK_PROVIDERS`, `ROVE_ROUTING_FAILURE_THRESHOLD`, and `ROVE_ROUTING_OPEN_COOLDOWN_MS` wire fallback model IDs, provider configs, and health policy into non-fake CLI/API OpenAI-compatible model construction; `src/errors.rs` adds stable `ModelError` codes plus retry and health-failure classification; provider adapters map common HTTP/provider errors into those variants | Failover is only attempted before the first response chunk is emitted; partial streams surface their original error instead of switching providers mid-response. Routing fallback construction is still OpenAI-compatible-provider oriented rather than native Anthropic/Ollama fallback config. |
...
| Routing config, probes, and circuit breaker | `docs/06` station 4 | A basic `RoutingModelClient` exists, `ROVE_FALLBACK_MODELS` and `ROVE_FALLBACK_PROVIDERS` route non-fake CLI/API OpenAI-compatible clients through it, first-chunk probe timeouts can fall through to the next candidate, repeated pre-commit failures open an in-memory circuit that later half-opens after cooldown, routing health threshold/cooldown are configurable, `ModelError` now exposes stable codes plus independent retry/health-failure classification, and OpenAI-compatible HTTP errors map auth/rate-limit/context-length responses into those variants. Missing: native non-OpenAI fallback adapters and deeper provider-specific mappings beyond the current OpenAI-compatible status/body classifier. |
...
| `request_input` interactive flow | `docs/06` station 7 | Tool surface, fallback output, `ToolContext` input provider trait, provider-backed direct tool answers, engine provider wiring, CLI stdin prompting, API pending-input response transport, and Web pending-input display/submission exist. Missing: broader interaction polish. |
...
| M1 core loop | Mostly implemented | Multi-step loop, file/shell tools, approval policy, hooks, state/report, context trimming, CLI fast paths, native Anthropic/Ollama adapters, basic routing client with first-chunk probing and configurable circuit state, OpenAI-compatible fallback provider config, model error classification, OpenAI-compatible HTTP error mapping, fake benchmarks/tests. Missing richer retry/time-budget behavior. |
...
| M4 MCP | Mostly implemented | Stdio MCP proxy, SSE proxy transport, and mock-server stdio test exist; real GitHub/filesystem server validation remains. |
...
| M6 Web UI | Implemented | Next.js workbench has chat, plan, tools, trace, cancel, approval controls, pending input submission, API job-state sync, responsive CSS polish, local run docs, production build coverage, and HTTP/SSE smoke coverage through the Next `/api` proxy. |

## This Turn's Follow-Up Work

This continuation advances the M6 workbench runtime polish:

- `web-ui/lib/rove-state.ts`: adds `job_state_synced` so the reducer can hydrate active job/run IDs, event counts, pending approvals, pending inputs, terminal statuses, and cancelled pending interactions from API state responses.
- `web-ui/components/rove-workbench.tsx`: consumes API state responses after cancel, approval, and pending-input submissions; SSE errors now trigger a state refresh from `/jobs/{id}/state`.
- `web-ui/lib/rove-state.test.ts`: covers pending interaction hydration and terminal cleanup from `JobStateResponse`.
- `web-ui/app/globals.css`: removes viewport-scaled/negative-letter-spacing heading typography and stacks pending-input controls on narrow viewports.
- `web-ui/README.md`: documents local API/Web startup, `ROVE_API_BASE`, production build, verification commands, and end-to-end smoke steps.
- Verification: `npm test`, `npm run typecheck`, `npm run build`, HTTP page/API/SSE smoke checks, and desktop/mobile Playwright screenshots pass for the Web UI after this slice.
...
- That slice left provider-specific fallback construction beyond same-base OpenAI-compatible model IDs.

This continuation wires station-4 provider-specific OpenAI-compatible fallbacks:

- `src/config.rs`: adds `ROVE_FALLBACK_PROVIDERS`, a JSON array of fallback provider configs with independent `api_base`, `api_key`, and `model`.
- `src/interfaces/cli/config.rs`: exposes fallback provider `api_base`, `model`, and `api_key_set` in `dump-config` without printing fallback API keys.
- `src/models/factory.rs`: adds configured fallback providers after same-base fallback model IDs when building a `RoutingModelClient`.
- Remaining station-4 gaps are native Anthropic/Ollama adapters and richer model error classification.

This continuation adds the first shared station-4 model error classification:

- `src/errors.rs`: adds stable `ModelError::error_code()`, `is_retryable()`, and `counts_as_health_failure()` helpers with unit coverage for all current variants.
- `src/models/routing.rs`: uses the shared health-failure classification instead of a routing-local helper.
- Remaining station-4 gaps are native Anthropic/Ollama adapters and richer provider-specific error-code mapping from HTTP/provider responses.

This continuation adds the first OpenAI-compatible provider error mapping:

- `src/models/openai.rs`: maps 401/403 to `AuthFailed`, 429 plus numeric `Retry-After` to `RateLimited`, and context-length status/body responses to `ContextLengthExceeded`, preserving OpenAI-style used/max token counts when available.
- `src/models/openai.rs`: routes non-success HTTP responses through the shared classifier before `RoutingModelClient` health decisions see the error.
- Remaining station-4 gaps are native Anthropic/Ollama adapters and deeper provider-specific mappings beyond the current OpenAI-compatible status/body classifier.
