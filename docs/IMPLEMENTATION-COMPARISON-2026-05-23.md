...
`main` is already beyond the original M0-M2 kernel plan and includes partial M3, M4, M5, and M6 surfaces. The largest difference is shape: the original docs describe an eventual `EngineDeps` dependency graph and richer provider routing, while the current implementation remains a simpler single-crate engine. `RunStream`, cooperative cancellation, CLI signal cancellation, ToolContext cancellation propagation, ToolContext input providers, engine-level input-provider wiring, CLI stdin `request_input`, API pending-input `request_input` transport, post-run hook cancellation, prompt-time durable memory, prompt-time session memory file loading, default post-run session summary writes, and a basic routing model client with fallback model-id config, first-chunk probing, in-memory circuit state, configurable health policy, OpenAI-compatible fallback provider configs, basic `ModelError` classification, and OpenAI-compatible HTTP error mapping now exist, but the full app-level token tree, layered memory store, durable-memory promotion hooks, user-configured post-run hooks, native Anthropic/Ollama adapters, and web input-response UI are still open.
...
| Model providers | OpenAI, Anthropic, Ollama, DeepSeek, routing/fallback | OpenAI-compatible client plus fake model | DeepSeek and other compatible providers can be configured through OpenAI-compatible bases; native Anthropic/Ollama adapters remain open. |
| Routing model client | Provider fallback with TTFB probe and circuit breaker | `src/models/routing.rs` adds a reusable fallback client with configurable first-chunk probe timeout and in-memory CLOSED/OPEN/HALF_OPEN circuit state; `ROVE_FALLBACK_MODELS`, `ROVE_FALLBACK_PROVIDERS`, `ROVE_ROUTING_FAILURE_THRESHOLD`, and `ROVE_ROUTING_OPEN_COOLDOWN_MS` wire fallback model IDs, provider configs, and health policy into non-fake CLI/API OpenAI-compatible model construction; `src/errors.rs` adds stable `ModelError` codes plus retry and health-failure classification; `src/models/openai.rs` maps auth, rate-limit, and context-length HTTP/provider errors into those variants, including retry-after and OpenAI-style context token counts when available | Failover is only attempted before the first response chunk is emitted; partial streams surface their original error instead of switching providers mid-response. Native non-OpenAI fallback adapters and deeper provider-specific error-code mapping remain open. |
...
| M1 core loop | Mostly implemented | Multi-step loop, file/shell tools, approval policy, hooks, state/report, context trimming, CLI fast paths, basic routing client with first-chunk probing and configurable circuit state, OpenAI-compatible fallback provider config, basic model error classification, OpenAI-compatible HTTP error mapping, fake benchmarks/tests. Missing native Anthropic/Ollama providers and richer retry/time-budget behavior. |
