...
| Routing model client | Provider fallback with TTFB probe and circuit breaker | `src/models/routing.rs` adds a reusable fallback client with configurable first-chunk probe timeout and in-memory CLOSED/OPEN/HALF_OPEN circuit state; `ROVE_FALLBACK_MODELS`, `ROVE_FALLBACK_PROVIDERS`, `ROVE_ROUTING_FAILURE_THRESHOLD`, and `ROVE_ROUTING_OPEN_COOLDOWN_MS` wire fallback model IDs, provider configs, and health policy into non-fake CLI/API OpenAI-compatible model construction; `src/errors.rs` adds stable `ModelError` codes plus retry and health-failure classification; `src/models/openai.rs` maps auth, rate-limit, and context-length HTTP/provider errors into those variants | Failover is only attempted before the first response chunk is emitted; partial streams surface their original error instead of switching providers mid-response. Native non-OpenAI fallback adapters and deeper provider-specific error-code mapping remain open. |
...
- `src/models/openai.rs`: maps 401/403 to `AuthFailed`, 429 plus numeric `Retry-After` to `RateLimited`, and context-length status/body responses to `ContextLengthExceeded`.
