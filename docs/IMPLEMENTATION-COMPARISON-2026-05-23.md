...
`main` is already beyond the original M0-M2 kernel plan and includes partial M3, M4, M5, and M6 surfaces. The largest difference is shape: the original docs describe an eventual `EngineDeps` dependency graph and richer provider routing, while the current implementation remains a simpler single-crate engine. `RunStream`, cooperative cancellation, CLI signal cancellation, ToolContext cancellation propagation, ToolContext input providers, post-run hook cancellation, prompt-time durable memory, prompt-time session memory file loading, default post-run session summary writes, and a basic routing model client now exist, but the full app-level token tree, layered memory store, durable-memory promotion hooks, user-configured post-run hooks, provider config wiring, and circuit-breaker design are still open.
...
| ToolContext input provider | Tools can receive an optional interface-backed input provider | `src/core/types.rs`, `src/tools/traits.rs`, `src/tools/registry.rs`, `src/tools/request_input.rs`, `tests/request_input_tool.rs` | The `request_input` tool returns provider answers when one is supplied; standard CLI/API engine construction still passes no provider. |
...
| `request_input` flow | `request_input` asks the user via `ToolContext` and returns the answer | Tool schema is registered in CLI/API; direct tool execution can return an answer from `ToolContext.input_provider`; default engine paths still return the clear interactive-provider-required fallback | Gives the LLM and tool boundary a stable contract, but real CLI/API user-response channels remain open. |
...
| `request_input` interactive flow | `docs/06` station 7 | Tool surface, fallback output, `ToolContext` input provider trait, and provider-backed direct tool answers exist. Missing: CLI/API provider wiring, response channel, and returning actual interface answers during engine runs. |
