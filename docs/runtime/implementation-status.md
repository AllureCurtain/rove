...
| Runtime core | `Engine` is public glue; shared model turns, tool turns, unplanned loop, planned coordinator, and bounded planned `StepRunner` live in focused core modules. A planned step can perform multiple model/tool turns, returns every tool result to the model, and completes only on a model step conclusion. Recoverable tool errors stay within the step; approval denial remains fail closed. The engine resolves legacy `plan_enabled`/`max_steps` through typed `ExecutionPolicy`; planned steps use an independent compatibility ceiling of four model turns. | The public multidimensional execution config, global model/tool/token accounting, structured budget events, append-only step ledger, immutable plan revision chain, evaluator, finalizer, and their persistence/report projections from the future lifecycle design are not implemented. |
