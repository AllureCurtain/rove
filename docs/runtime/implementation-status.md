...
| `rove tui` reuses the shared engine/events/artifacts, handles live approval/input through direct enhanced-terminal actions or the Windows F8 safety path, fails closed otherwise, keeps the full-screen session open after run completion/cancellation, and restores the terminal when the TUI exits, its loop errors, or panic unwinds. | Met by `cargo test interfaces::tui --lib`, `cargo test interfaces::terminal --lib`, the TUI fake-provider artifact test, fail-closed interaction tests, and the destructive-tool rejection test; PTY and cross-platform real-terminal coverage remain pending |
