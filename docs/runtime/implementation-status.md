...
| Repository packaging | Root `Cargo.toml` is a transitional resolver-3 Workspace with the existing root `rove` package as its only member/default member. Workspace package metadata and dependency versions are centralized. External-style embedding, old-artifact, event-contract, and metadata dependency-direction guards run as root integration tests. | The modular packages and final virtual root have not yet been extracted; current source ownership remains under root `src/`. |
