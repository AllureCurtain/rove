...
| Interface runtime assembly | CLI uses a sync fast path for `dump-config`/help before Tokio startup. CLI and API share the async runtime tool registry builder, including configured MCP tools. | None for the current interface parity target. |
...
| CLI fast paths avoid async runtime startup where practical. | Met for `dump-config` and help |
