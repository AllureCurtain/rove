...
| Interface runtime assembly | CLI uses a sync fast path for `dump-config` before Tokio startup. Running `rove` with no task enters a line-oriented REPL; quoted and unquoted multi-word tasks run as one-shot messages. Optional `rove tui` uses the same `CliRuntime`, `Engine`, canonical events, run artifacts, and cancellation token through an alternate-screen Ratatui/Crossterm shell. CLI and API share the async runtime tool registry builder, including configured MCP tools. | The first TUI slice is single-session and has no approval/input modal, session picker, or PTY smoke gate yet. |
| Full-screen TUI vertical slice | `rove tui` has a reducer-driven EventStream loop, bounded async run projection, wrapped transcript/history, cancellation, confirmation-before-exit, resize/narrow rendering, and RAII terminal restoration. The shared driver drains post-run hooks and finalizes artifacts before publishing completion. | Approval/input modals, session browsing, strict chronological transcript entries, and PTY-level interaction tests remain follow-up milestones. |
...
| `rove tui` first vertical slice reuses the shared engine/events/artifacts and restores the terminal on completion, cancellation, error, and panic unwind. | Met by `cargo test interfaces::tui --lib`, `cargo test interfaces::terminal --lib`, and the TUI fake-provider artifact test; PTY coverage remains pending |
