...
| Model-call retry budget with canonical visibility | Implemented: the run loop retries a model call that failed before producing output, with independent rate-limit and transient budgets, exponential backoff capped at 30s, honoured and clamped provider `retry_after_ms`, a cancellable wait, and a canonical `provider_retry` event whose `reason` is a whitelist; a turn that already streamed `LlmChunk` output is never retried; an error with no retry class fails on its first attempt; the event reaches `trace.jsonl` and the SQLite event index; the default budget is enabled and `max_attempts = 1` on both classes disables it | `cargo test -p rove-integration-tests --test recovery`; `a_scheduled_retry_reaches_the_durable_trace_and_the_event_index` in `tests/e2e.rs`; `cargo test -p rove-runtime --lib engine::run_loop`; `cargo test -p rove-runtime --lib engine::recovery`; `cargo test -p rove-app-bootstrap --lib config::tests`; `cargo test -p rove-integration-tests --test event_contract`; `cd apps/web; pnpm test` |
| Abort salvage for a cancelled turn that already published text | Implemented (R2b): cancelling a run whose model turn already published `LlmChunk` text waits at most `ABORT_SALVAGE_WINDOW` (1500 ms, a runtime constant, not configuration) for the in-flight request; a message that completes inside the window is persisted with `aborted = false`, an expired window persists the accumulated text with `aborted = true`, and the run still ends as cancelled. Both branches write one canonical `StreamEvent::LlmMessage`, which projects the assistant message into resumable history and the trace fact, so the surfaces cannot disagree. `aborted` is additive with `#[serde(default)]` (a legacy trace line or SSE frame reads as complete), the resumable-history projection drops the marker, a Review-mode turn never publishes `LlmChunk` and therefore never salvages, a cancel with no published text still writes nothing, repeated cancellation cannot write twice, and the tool/approval path is untouched because the window waits for text only and salvaged text carries no tool calls | `cargo test -p rove-integration-tests --test abort_salvage` (`a_cancelled_turn_keeps_the_text_the_user_already_saw`, `a_request_that_finishes_inside_the_window_keeps_a_complete_message`, `a_cancel_with_no_accumulated_text_still_writes_nothing`, `a_second_cancel_neither_restarts_the_window_nor_writes_twice`, `a_cancel_during_a_tool_call_never_salvages`, `resume_sees_a_salvaged_message_once_and_never_replays_it`, `a_trace_line_without_the_field_is_a_complete_message`, `the_salvage_window_is_bounded`); `cargo test -p rove-core --lib model_turn`; `cargo test -p rove-api --lib sse_frame_carries_the_aborted_marker_on_llm_message`; `cd apps/web; pnpm test` |
...
| F6 Smart-stop send snapshot | Implemented: an accepted send keeps the draft as it was written — the typed text plus the paste chips that the outgoing message folded into its text. A stop that cut the turn before the model produced anything restores that snapshot once (text and chips), and both the composer notice and the paste notice say the draft came back from the snapshot; with no snapshot (for example after a reload, because drafts are memory-only) the stop falls back to the folded message text and says that instead, dropping any chip that no longer matches. The snapshot is dropped when the turn reaches its terminal state. The runtime now publishes the R2b `aborted` marker and the transcript renders it as a per-message suffix; the smart-stop partial-abort copy branch stays reserved because the stop handler runs while the salvage window is still open (frontend design §7.5) | `apps/web/state/composer-draft-store.test.ts`; `apps/web/chat/smart-stop.test.ts`; `apps/web/chat/Transcript.test.tsx`; `apps/web/tests/e2e/smart-stop-snapshot.spec.ts` |
