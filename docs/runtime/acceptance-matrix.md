...
| Desktop D0 plus independent real-use D1-D5 slice | D0 remains implemented. The current Desktop slice adds a native Windows keyring prompt with safe receipts, typed WebView wrapper, redacted startup failure path, authenticated SSE replay regression, and per-machine MSI/NSIS packages with a Start menu folder. Package build passed. Shared Catalog onboarding publication, installation, credentialed SiliconFlow tasks, restart evidence, and D6 remain unverified. | `cargo test -p rove-desktop --all-targets`; focused Desktop Web tests; `pnpm typecheck`; Windows `pnpm dlx @tauri-apps/cli@2 build --bundles "msi,nsis" --ci`; `desktop-real-use.md` |
