...
| M2 | Planner supports persisted plans, replanning, shared model/tool execution paths, resume without repeating completed steps, and context checkpoints. | Met | `cargo test --test e2e planner_replans_after_step_failure -- --exact`; `cargo test --test e2e planner_resume_checkpoint_does_not_repeat_completed_steps -- --exact`; `cargo test --test e2e planned_and_unplanned_runs_emit_equivalent_tool_lifecycle_events -- --exact` | Closed by phases 1, 5, 6 |
...
| M6 | Standalone Web workbench consumes the API/SSE stream, supports approval/input/cancel/resume flows, token proxying, and browser E2E coverage. | Met | `cd web-ui; npm test`; `cd web-ui; npm run typecheck`; `cd web-ui; npm run build`; optional browser check: `cd web-ui; npm run test:e2e` | Closed by phases 3, 8 |
