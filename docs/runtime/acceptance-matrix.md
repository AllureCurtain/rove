...
| Model-call retry budget with canonical visibility | Implemented: the run loop retries a model call that failed before producing output, with independent rate-limit and transient budgets, exponential backoff capped at 30s, honoured and clamped provider `retry_after_ms`, a cancellable wait, and a canonical `provider_retry` event whose `reason` is a whitelist; a turn that already streamed `LlmChunk` output is never retried; `max_attempts = 1` disables it | `cargo test -p rove-integration-tests --test recovery`; `cargo test -p rove-runtime --lib engine::run_loop`; `cargo test -p rove-runtime --lib engine::recovery`; `cargo test -p rove-app-bootstrap --lib config::tests`; `cargo test -p rove-integration-tests --test event_contract`; `cd apps/web; pnpm test` |
