...
| Silent-turn recovery with a single-attempt budget | Implemented: the React host recovers a run that terminated as `Final` with no assistant text, no `ToolCallStarted`, and a real user message by spending at most one extra model turn with the fixed nudge injected as a system message; the recovery turn is ordinary (same budgets, tools, approvals, cancellation, repair limits) and a second silent turn terminates exactly as before; announced as `model_status` `recovering_silent_turn` carrying the nudge and recorded as an `execution_degraded` fact with code `silent_turn_recovery` that materializes into the persisted lifecycle degradations; both reach `trace.jsonl` and the SQLite event index; `silent_turn_max_attempts = 0` restores the pre-recovery event stream exactly; a run that already used a tool or has no user message is never silent; the Web narrates the runtime's own message and keeps the model phase | `a_silent_turn_is_recovered_once_with_the_fixed_nudge`, `two_silent_turns_terminate_after_one_recovery_attempt`, `zero_attempts_reproduces_the_pre_recovery_stream_exactly`, `a_recovery_turn_moves_through_the_normal_tool_path`, `cancellation_during_the_recovery_turn_cancels_normally`, `a_tool_using_run_that_ends_empty_is_not_recovered`, `a_silent_run_with_no_user_message_is_not_recovered`, and `a_recovered_run_is_durable_and_resumable` in `tests/recovery.rs`; `cargo test -p rove-runtime --lib engine::recovery`; `cargo test -p rove-app-bootstrap --lib config::tests`; `apps/web/chat/activity-phase.test.ts`; `apps/web/lib/rove-state.test.ts` |
