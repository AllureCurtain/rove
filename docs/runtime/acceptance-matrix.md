...
| Fake-provider runtime evidence is separated from native-provider interoperability evidence. | Fake-provider evidence remains deterministic and separate; the credentialed SiliconFlow TUI gate is recorded independently, while Web/Desktop external-provider gates remain unverified. | `cargo test -p rove-integration-tests --test bench oncall_benchmark_v2_passes_independent_truth_and_hard_safety_gates -- --exact`; `cargo run -p rove-bench -- --suite benchmarks/agent-smoke.json --output-dir .rove/bench`; credentialed TUI evidence is listed in the final real-use slice below. |
...
| Real Provider can complete the credentialed multi-turn/tool interoperability gate | Met for the installed TUI with SiliconFlow `deepseek-ai/DeepSeek-V3.2`; Web/Desktop equivalence remains unverified | Follow `provider-smoke.md`; credentialed evidence `<evidence-root>/tui-gate-10` records three successful turns with native tool calls |

## Productization F (Unified Conversation Control)

| Contract | Current status | Verification surface |
|---|---|---|
| One durable Send Message command with FIFO, idempotency, promotion/revoke CAS, and six delivery states | Implemented in Runtime schema v3 and ProductStore schema v14; v13 reconciles both parallel v12 layouts and v14 adds Review tables without changing the message contract | `cargo test -p rove-runtime conversation::tests::sqlite_adapter_is_fifo_idempotent_and_cas_safe --lib`; ProductStore migration tests; `cargo test -p rove-api unified_message --lib` |
...
| Conversation-first Web transcript with streaming follow, exact `(job_id, run_id)` attachment, and delivery-state actions | Core lifecycle implemented and verified by unit/type/build, mocked browser, and live local fake-provider gates; F.4 older-history pagination and stable prepend anchoring are implemented through the transcript cursor and `load older history`; bounded lazy rendering serves the long session, while TUI long-session prepend (F.5) and the live-API pagination journey remain open | `apps/web/chat/*`, `apps/web/state/*`, `apps/web/api/run-controller.test.ts`, `pnpm test`, `pnpm typecheck`, `pnpm build`, `pnpm test:e2e`, `scripts/integration-smoke.ps1` |
...
| Contract | Current status | Verification evidence |
|---|---|---|
| Immutable Git target capture for uncommitted, base, and commit targets with staged/worktree facts, bounded diffs, content hashes, and stale detection | Implemented in `runtime::review`; base targets resolve and read the requested base object instead of silently using `HEAD` | `cargo test -p rove-runtime review -- --nocapture`; `cargo test -p rove-integration-tests --test review -- --nocapture` |
| Review execution cannot mutate the target workspace, Git index, memory, MCP, hooks, or external processes | Implemented with a dedicated registry, read-only `ExecutionEnvironment`, pre-dispatch allowlist, `ApprovalPolicy::Never`, and no MCP registration; automatic read-output artifact retention is disabled in Review mode | `cargo test -p rove-runtime tools::executor::tests::review_read_output_is_not_retained_as_a_durable_tool_artifact -- --exact`; `cargo test -p rove-integration-tests --test tool_safety`; Review integration source/digest assertions |
| Versioned, bounded, path/line validated, deduplicated, and redacted findings with conservative conclusions (`pass`, `findings`, `partial`, `stale`, `unavailable`, `cancelled`, `error`) | Implemented and projected through one Runtime result contract; no-finding plus unchecked ranges remain distinct | `cargo test -p rove-runtime review -- --nocapture`; `cargo test -p rove-integration-tests --test cli_review -- --nocapture` |
| API/ProductStore lifecycle, idempotency, pagination, cancellation, stale recheck, and restart recovery | Implemented in ProductStore schema v14; reopening the store classifies stranded queued/running Reviews as `needs_attention` | `cargo test -p rove-api product::store::schema -- --nocapture`; `cargo test -p rove-api product::store::tests::reopening_the_store_marks_interrupted_reviews_needs_attention -- --exact`; `cargo test -p rove-integration-tests --test review -- --nocapture` |
| CLI text/JSON/JSONL and Web composer/Inspector states | Implemented; Web long titles and paths have bounded flex/grid behavior. Browser Review-specific E2E remains unrun; existing mocked Web suites pass | `cargo test -p rove-integration-tests --test cli_review -- --nocapture`; `cd apps/web; pnpm test`; `cd apps/web; pnpm typecheck`; `cd apps/web; pnpm build` |
...
| Bounded transcript cursor paging over canonical run bindings | Implemented: `before_ordinal`/`limit_runs` pages of whole runs, additive `next_before_ordinal`/`has_more`, unchanged parameterless response, typed `product_invalid_input` bounds, partial page plus continued paging when a page exhausts the response budget | `product_transcript_cursor_pages_older_runs_without_gaps_or_repeats`, `product_transcript_page_query_rejects_unbounded_pages`, and the OpenAPI parameter assertion in `tests/api.rs`; `PageWalk` unit tests in `apps/api/src/product/transcript/reader.rs`; `apps/web/tests/e2e/transcript-pagination.spec.ts` |
...
| Session outcome of the most recently finished turn | Implemented: ProductStore v16 adds nullable `last_outcome`/`last_outcome_at` with a `success`/`failed`/`cancelled` CHECK and no backfill; the closing transaction writes both, an attempt that is undone before it becomes a turn writes neither, and the projection omits both fields together for a session that never finished a turn | `a_v15_store_gains_the_session_outcome_columns_without_backfilling`, `session_outcomes_are_constrained_and_the_upgrade_is_idempotent`, and `finishing_a_turn_records_the_session_outcome_in_the_same_transaction` in `apps/api/src/product/store/`; the cancel/attention/follow-up assertions and the `ProductSession` OpenAPI field assertion in `tests/api.rs`; `apps/web/product/product-api-types.test.ts`, `apps/web/state/product-catalog.test.ts`, `apps/web/sidebar/session-labels.test.ts`, and the sidebar-outcome case in `apps/web/tests/e2e/continuity.spec.ts` |
...
| Desktop D0 plus real-use D1-D5 slice with shared onboarding | D0 remains implemented. The Desktop slice now completes the native create/test/use Provider flow through the shared `ProviderOnboardingService` via an in-process `ApiState` facade, with keyring storage, real inventory probe, Catalog CAS publication, and failure compensation owned by the shared service. The Desktop-private keyring path was removed; no HTTP route accepts a secret. Typed WebView wrapper, redacted startup failure path, authenticated SSE replay regression, and per-machine MSI/NSIS packages with a Start menu folder remain. Installation, credentialed SiliconFlow tasks, restart evidence, and D6 remain unverified. | `cargo test -p rove-api` (137); `cargo test -p rove-desktop --all-targets` (13 lib + 3 integration); `secure_onboarding_projects_catalog_identity_without_serializing_the_secret` and `onboarding_errors_are_typed_and_redacted` in `apps/api/src/product/provider_onboarding.rs`; `probe_and_use_metadata_are_bounded_and_secret_free` in `apps/desktop/src/commands.rs`; `apps/web/platform/desktop-commands.test.ts`; `pnpm test`/`typecheck`/`build:desktop`; Windows `pnpm dlx @tauri-apps/cli@2 build --bundles "msi,nsis" --ci`; `desktop-real-use.md` |
...
| F3 Streaming markdown segment memo | Implemented: each segment renders through a memoized markdown component compared on its own text, so the segments a closed fence already finished are not re-parsed as deltas arrive; a prose-only reply is still one growing segment, which is the recorded limit because rove splits only at fences | `apps/web/product-v2/rich-text-streaming.test.tsx` (render counts in jsdom); `apps/web/chat/streaming-blocks.test.ts`; `apps/web/tests/e2e/rich-text-segments.spec.ts` |
| F4 Session hover card | Implemented: a sidebar session row hovered for 300 ms shows a read-only tooltip card inside the shell frame with the session's status, timestamps, fork source, workspace root and its lazily read model config; leaving closes it, Escape closes it for the rest of the hover, and keyboard focus never opens it | `apps/web/sidebar/session-hover-card.test.ts`; `apps/web/sidebar/session-model-summary.test.ts`; `apps/web/tests/e2e/session-hover-card.spec.ts` |
| F5 Toast notification layer | Implemented for the toast layer and background failure visibility: at most three toasts share the screen (oldest leaves first), success/info stay 4 s and errors 8 s, hovering pauses the timers, errors are also kept in a bounded queryable history (the seam the runtime R5 inbox reuses), and reduced motion shows them without the slide. A session that moves into `error` while background polling runs raises one toast inside a five-minute per-session window; a failure first seen already failed is not news, and the session being read reports itself inline. The notification inbox itself stays blocked on runtime R5 | `apps/web/shell/toast/toast-store.test.ts`; `apps/web/shell/toast/notifications.test.ts`; `apps/web/tests/e2e/toast.spec.ts` |
| F6 Smart-stop send snapshot | Implemented: an accepted send keeps the draft as it was written — the typed text plus the paste chips that the outgoing message folded into its text. A stop that cut the turn before the model produced anything restores that snapshot once (text and chips), and both the composer notice and the paste notice say the draft came back from the snapshot; with no snapshot (for example after a reload, because drafts are memory-only) the stop falls back to the folded message text and says that instead, dropping any chip that no longer matches. The snapshot is dropped when the turn reaches its terminal state. The partial-abort copy branch stays reserved for the runtime R2b `aborted` marker and changes nothing until that marker exists | `apps/web/state/composer-draft-store.test.ts`; `apps/web/chat/smart-stop.test.ts`; `apps/web/tests/e2e/smart-stop-snapshot.spec.ts` |
| F7 Misc cleanup | Implemented: the `/dev/workbench` banner no longer claims a benchmark Settings section that does not exist, only that the route is a development one. The inert `product-ui-v2` design mock keeps its notFound dev gate and now states "Design mock, not product." above everything. The message-level fork button keeps its honest copy-session label (a fork anchors the last terminal run's sequence, which the server derives), and the session-header fork button uses the same copy key instead of a hardcoded `Fork`. Message-level edit/resend stays with runtime R6 | `apps/web/tests/e2e/product-ui-v2.spec.ts`; `apps/web/app/dev/product-ui-v2/ProductUiV2Preview.test.tsx` |
| F8 Style-token linter | Implemented as a local gate: `scripts/check-web-style-tokens.mjs` (with `node --test` self-tests) rejects transition/animation durations and timing functions in `apps/web/styles/**/*.css` that bypass `var(--motion-duration-*)` / `var(--motion-ease-*)`, exempts `prefers-reduced-motion` blocks and declaration-level reasoned `/* style-token: allow ... */` comments, and reports an exemption that has no reason or exempts nothing. The first run found 29 existing literals: the v2 sheet now uses tokens (six new loop/collapse duration tokens and two new ease tokens; values unchanged except a 140ms hover becoming the existing 150ms fast token) and the frozen v1 sheet carries four reasoned exemptions. Wired as `pnpm lint:style-tokens` from `apps/web`; deliberately not a CI gate | `scripts/check-web-style-tokens.test.mjs`; `pnpm lint:style-tokens` |
| F9 Rail collapse motion | Implemented after measuring first (CDP `Performance.getMetrics`, 1280x720, 30-run transcript, 6 toggles): animating the grid track cost 15.2 layout passes / 18.9ms layout / 33.4ms style recalculation per collapse against 2 / 1.8ms / 6.0ms once the track settles in one frame. `grid-template-columns` no longer transitions; the rail keeps its own 240ms `transform + opacity` slide and therefore gets a stable `width: var(--sidebar-nav-width)` with `position: relative; z-index: 2` so it can slide out over the conversation instead of being squeezed to zero with the track. Recorded deviation: the conversation switches instantly instead of being pushed across, and the off-screen rail was already `inert` + `aria-hidden` | `apps/web/tests/e2e/layout.spec.ts`; design §10.4 |
