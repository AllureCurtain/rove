...
### 验收
- [x] 10k 假想会话（生成 fixture）下列表 50ms 预算（断言从 p95 改为中位数，见补充说明）；
- [x] 游标翻页无重复无遗漏（属性测试）。
...
| 验收项 | 证据 | 结果 |
| --- | --- | --- |
| 10k fixture 下 50ms 预算 | `product::store::pagination_tests::paging_deep_into_a_ten_thousand_session_workspace_stays_flat`：10 000 条会话、连翻 60 页（每页 50 条）。断言落在**中位数**而非 p95（原因见下方补充说明），实测中位 9.32ms、p95 19.17ms，前十页合计 109.4ms、后十页合计 109.8ms（越翻越深不变贵） | 通过 |
...
> 诚实性说明：这条验收项是预算检查，不是分页设计的证明。实测确认它**抓不到**两件事：把排序改回去只让单页贵约五成（远在任何能在共享机器上稳定通过的阈值之内）；而且排序形态下页延迟**同样**与深度无关（排的是单个 rank 组，组大小不随翻到多深而变），所以"深页不比浅页贵"的比值断言也分不开两种形态。真正的保证是查询计划测试。这一点已写进测试的文档注释，比值只打印不断言。
>
> **补充（2026-08-29，合并后修复不稳定）：断言从 p95 改为中位数。**
> 该测试在 main 上合并后首次全量跑即失败，且同一 commit 上时过时不过——原因是整个 suite 在单机并行，p95 落在被调度噪声支配的尾部。改成中位数后连跑 3 轮全量并行稳定通过：失败中位数意味着**过半页**超预算，那是回归而不是抖动；p95 仍然打印，因为读失败时想看的是它。
>
> 修这条时顺带量到一组数字，值得记下来，因为它把这个测试能防守的范围划得更清楚了：索引 seek 单页约 **0.7ms**，全表扫 11k 行约 **1.8ms**，而一次 `list_sessions` 端到端约 **7ms**——绝大部分是固定的每查询开销。**噪声底噪比整个信号还大**，所以这里任何计时断言（绝对值或比值）都分不开 seek 与 scan。我实测验证过：把两个索引全删掉，中位页成本相对十倍小的工作区只从 1.03x 变到 1.19x，没有任何诚实的阈值能抓住。
>
> 这也意味着我一度打算改成的"规模不变性比值断言"同样立不住，已放弃。留下的形态是：查询计划测试守访问形状，这条计时测试只守数量级灾难（比如每行引入一次 N+1、或每页重建一次）。上述数字与结论已写进测试的文档注释，避免下一个人重走一遍。
