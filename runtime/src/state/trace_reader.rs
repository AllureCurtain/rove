...
//!
//! New traces carry every line inside a [`TraceLine`] envelope (`{ts, seq,
//! event}`). Legacy traces contain bare `StreamEvent` objects with no
//! sequence. This reader accepts both formats in the same file (lazy
//! upgrade: old lines keep their line-number-derived sequence), skips a
//! truncated final line left behind by a crash, and reports what it saw so
//! callers can surface degraded reads instead of failing.
use std::path::Path;
...
/// One decoded trace entry with its effective sequence number.
#[derive(Debug, Clone)]
...
impl TraceReadOutcome {
    /// Sequence continuity check over the decoded entries.
    ///
    /// Mixed legacy/envelope files may legitimately interleave sequences, so
    /// this only asserts that the run is *replayable*: entries are ordered by
    /// file position and no entry is duplicated within one format generation.
...
fn parse_line(line: &str, fallback_seq: u64) -> std::io::Result<TraceEntry> {
    // New format first.
    if let Ok(enveloped) = serde_json::from_str::<TraceLine>(line) {
        return Ok(TraceEntry {
            seq: enveloped.seq,
            ts: Some(enveloped.ts),
            event: enveloped.event,
        });
    }
    // Legacy bare-event fallback.
    match serde_json::from_str::<StreamEvent>(line) {
        Ok(event) => Ok(TraceEntry {
            seq: fallback_seq,
            ts: None,
            event,
        }),
...
/// Read and decode a whole trace file, tolerating both line generations and a
...
mod tests {
    ...
    fn new_format_lines_carry_ts_and_seq() {
        ...
        insta::assert_debug_snapshot!(
            outcome
                .entries
                .iter()
                .map(|entry| (entry.seq, entry.ts.clone(), entry.event.event_name()))
                .collect::<Vec<_>>()
        );
    }
    ...
    fn legacy_bare_event_lines_fall_back_to_line_number_seq() {
        ...
        assert_eq!(outcome.entries[0].seq, 1);
        assert_eq!(outcome.entries[1].seq, 2);
        assert!(outcome.entries.iter().all(|entry| entry.ts.is_none()));
    ...
    fn writer_assigns_continuous_seq_from_memory_counter() {
        ...
        let seqs: Vec<u64> = outcome.entries.iter().map(|entry| entry.seq).collect();
        assert_eq!(seqs, vec![1, 2]);
        assert!(
            outcome
                .entries
                .iter()
                .all(|entry| entry.ts.as_ref().is_some_and(|ts| !ts.is_empty()))
        );
